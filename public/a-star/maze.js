	...
	while (openSet.length > 0) {
		...
		if (current == end) {
			...
			while (temp.previous != start) {
				...
				temp = temp.previous;
				temp.highlight("rgba(255, 255, 0, .2)");
			}

			let totalTime = new Date().getTime() - startTime;
			console.log("Done! It took " + (totalTime.toFixed(0) / 1000) + " seconds to find the path.");
		...
		// Add array to array :)
		// openSet.push(...current.getNeighbors());
		...
		// console.log("Start: ", start, "End: ", end, openSet, counter);
	}
}
...
function drawPathLine(path, done) {
	...
	if (done) {
		if (path[0].x <= 0) {
			context.moveTo((path[0].x * cellSize), (path[0].y * cellSize) + (cellSize / 2));
		}
		else if (path[0].y <= 0) {
			context.moveTo((path[0].x * cellSize) + (cellSize / 2), (path[0].y * cellSize));
		}
		else if (path[0].x >= cols - 1) {
			context.moveTo(((path[0].x + 1) * cellSize), (path[0].y * cellSize) + (cellSize / 2));
		}
		else if (path[0].y >= rows - 1) {
			context.moveTo((path[0].x * cellSize) + (cellSize / 2), ((path[0].y + 1) * cellSize));
		}
		context.lineTo((path[0].x * cellSize) + (cellSize / 2), (path[0].y * cellSize + (cellSize / 2)));
	}
	else {
		context.moveTo((path[0].x * cellSize) + (cellSize / 2), (path[0].y * cellSize + (cellSize / 2)));
	}
	...
	if (start.x <= 0) {
		context.lineTo((start.x * cellSize), (start.y * cellSize) + (cellSize / 2));
	}
	else if (start.y <= 0) {
		context.lineTo((start.x * cellSize) + (cellSize / 2), (start.y * cellSize));
	}
	else if (start.x >= cols - 1) {
		context.lineTo(((start.x + 1) * cellSize), (start.y * cellSize) + (cellSize / 2));
	}
	else if (start.y >= rows - 1) {
		context.lineTo((start.x * cellSize) + (cellSize / 2), ((start.y + 1) * cellSize));
	}
